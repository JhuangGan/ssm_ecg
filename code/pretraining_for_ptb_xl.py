# 用于将ptb_xl_fs500的训练集用来预训练的脚本，即对205行的，max_fold与df_train,df_val做调整

# --finetune --finetune-dataset，算了不用这个

# python code/pretraining.py --data ./data/ptb_xl_fs500 --normalize --epochs 200 --output-path=./runs/cpc/all/ptb_xl_fs500_pretrain --lr 0.0001 --batch-size 32 --input-size 1000 --fc-encoder --negatives-from-same-seq-only --mlp --s4 --precision 32 --single_gpu_choose 3
...
class LightningCPC(pl.LightningModule):
    ...
    def setup(self, stage):
        ...
        for i,target_folder in enumerate(self.hparams.data.split(",")):
            ...
            # max_fold_id = 10  ## 修改为10，以小于9的，即1-8的训练集为训练，等于9的为验证集，也就是上面的最大值，设置finetune超参数
